...
async function seedMetric(metric: MetricDef, geo: "county" | "zip") {
  ...
  console.log(`  Parsed ${rows.length} regions, ${dateColumns.length} dates`);

  // Only seed latest date to keep it fast
...
async function main() {
  console.log("╔══════════════════════════════════════════╗");
  console.log("║  Zillow Metrics Seeder (Latest Month)    ║");
  console.log("╚══════════════════════════════════════════╝");

  const startTime = Date.now();
  const onlyMetric = process.argv[2]; // Optional: seed only one metric

  for (const metric of METRICS) {
    if (onlyMetric && metric.key !== onlyMetric) continue;

    console.log(`\n═══ ${metric.label} (${metric.key}) ═══`);
    await seedMetric(metric, "county");
    await seedMetric(metric, "zip");
  }
